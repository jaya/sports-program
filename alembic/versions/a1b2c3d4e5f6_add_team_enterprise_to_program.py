...
def upgrade() -> None:
    ...
    op.add_column('programs', sa.Column(
        'enterprise_id', sa.String(), nullable=True))
    op.create_index(op.f('ix_programs_team_id'),
                    'programs', ['team_id'], unique=False)
    op.create_index(op.f('ix_programs_enterprise_id'),
                    'programs', ['enterprise_id'], unique=False)
