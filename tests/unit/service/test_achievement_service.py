...
@pytest.mark.anyio
async def test_notify_no_channel_configured(service):
    with patch("app.services.achievement_service.settings") as mock_settings:
        mock_settings.SLACK_NOTIFICATION_CHANNEL = ""

        with pytest.raises(BusinessRuleViolationError) as exc_info:
            await service.notify(program_name="Test", cycle_reference="2023-10")

        assert "SLACK_NOTIFICATION_CHANNEL" in str(exc_info.value.message)


@pytest.mark.anyio
async def test_notify_program_not_found(service, mock_program_repo):
    with patch("app.services.achievement_service.settings") as mock_settings:
        mock_settings.SLACK_NOTIFICATION_CHANNEL = "C123"
...
async def test_notify_success(service, mock_program_repo, mock_achievement_repo):
    with patch("app.services.achievement_service.settings") as mock_settings:
        with patch("app.services.achievement_service.slack_app") as mock_slack:
            ...
            mock_achievement_repo.mark_as_notified.assert_called_once_with([
                                                                           1, 2])
