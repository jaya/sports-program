...
from app.schemas.activity import ActivitySummaryResponse, ActivityUpdate
...
class Update:
    ...
    async def execute(
        ...
        if (
            ...
            self.validate_performed_at(program_found, activity_update.performed_at)
            await self.check_activity_same_day.execute(
                program_found.id, user_id, activity_update.performed_at, id
            )
        ...
        total_month = await self.count_service.execute(
            user_id=user_id, performed_at=db_activity.performed_at
        )
